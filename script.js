document.addEventListener('DOMContentLoaded', () => {
  ...
  if (contactServiceSelect && contactTypeSelect) {
    ...
    contactServiceSelect.addEventListener('change', (e) => {
      ...
      contactTypeSelect.innerHTML = '<option value="">Select a Service Name</option>';
      const bLocation = document.getElementById('b-location');
      if (bLocation) {
        bLocation.innerHTML = '<option value="">Select a Service first...</option>';
        // Trigger change to hide branch/address
        bLocation.dispatchEvent(new Event('change'));
      }
    ...
    contactTypeSelect.addEventListener('change', (e) => {
      ...
      const selectedCategoryName = contactServiceSelect.value;
      const bLocation = document.getElementById('b-location');
      
      if (bLocation) {
        bLocation.innerHTML = '';
        if (selectedServiceName && selectedCategoryName) {
          const category = categoriesData.find(c => c.categoryName === selectedCategoryName);
          const service = category.services.find(s => s.name === selectedServiceName);
          if (service && service.location) {
            if (service.location.toLowerCase().includes('parlour')) {
              bLocation.add(new Option('At Parlour', 'At Parlour'));
            }
            if (service.location.toLowerCase().includes('on-site')) {
              bLocation.add(new Option('On-Site / Home Service', 'On-Site'));
            }
          }
        } else {
          bLocation.innerHTML = '<option value="">Select a Service first...</option>';
  ...
  function setupLocationLogic(locationSelectId, branchGroupId, branchSelectId, addressGroupId, addressInputId, landmarkInputId, cityInputId, paymentNoteId) {
    const locationSelect = document.getElementById(locationSelectId);
    const branchGroup = document.getElementById(branchGroupId);
    ...
    function applyAtParlourLogic() {
      if (addressGroup) addressGroup.style.display = 'none';
      if (addressInput) { addressInput.required = false; addressInput.value = ''; }
      if (landmarkInput) { landmarkInput.required = false; landmarkInput.value = ''; }
    ...
    if (locationSelect && addressGroup && addressInput && paymentNote) {
      locationSelect.addEventListener('change', (e) => {
        if (e.target.value === 'On-Site') {
          ...
          showCustomConfirm(
            "For On-Site service, 50% advance payment is required to confirm your booking. After submitting your booking request, the admin will send the payment scanner/QR code from this WhatsApp number only: 7022159756. Please make payment only after receiving the scanner from this number.",
            () => {
              // I Understand
              if (branchGroup) branchGroup.style.display = 'none';
              if (branchSelect) { branchSelect.required = false; branchSelect.value = ''; }

              addressGroup.style.display = 'block';
              addressInput.required = true;
              if (landmarkInput) landmarkInput.required = true;
              if (cityInput) cityInput.required = true;

              paymentNote.innerHTML = '<strong>📱 On-Site Payment:</strong> 50% advance payment required. Admin will send payment scanner from 7022159756 only.';
            },
            () => {
              // Cancel
  ...
  setupLocationLogic('m-location', 'm-branch-group', 'm-branch', 'm-address-group', 'm-address', 'm-landmark', 'm-city', 'm-payment-note');
  setupLocationLogic('b-location', 'b-branch-group', 'b-branch', 'b-address-group', 'b-address', 'b-landmark', 'b-city', 'b-payment-note');
  ...
  if (modalBookingForm) {
    ...
    modalBookingForm.addEventListener('submit', (e) => {
      ...
      const branch = document.getElementById('m-branch') ? document.getElementById('m-branch').value : '';
      ...
      const address = document.getElementById('m-address') ? document.getElementById('m-address').value : '';
      const landmark = document.getElementById('m-landmark') ? document.getElementById('m-landmark').value : '';
      const city = document.getElementById('m-city') ? document.getElementById('m-city').value : '';
      const date = document.getElementById('m-date').value;
      const time = document.getElementById('m-time').value;
      const msg = document.getElementById('m-msg').value;

      const whatsappNumber = '917022159756';

      let text = `Hello, I want to book an appointment at LOTUS BEAUTY ZONE 💄✨\n\n`;
      text += `Service: ${serviceType}\n`;
      text += `Service Type: ${location === 'On-Site' ? 'On-site / Home Service' : 'At Parlour'}\n`;
      if (location !== 'On-Site' && branch) {
        text += `Branch: ${branch}\n`;
      }
      if (location === 'On-Site' && address) {
        text += `Customer Address: ${address}, ${landmark}, ${city}\n`;
      }
      text += `Name: ${name}\n`;
      text += `Phone: ${phone}\n`;
      text += `Date: ${date}\n`;
      ...
      let noteText = location === 'On-Site' 
        ? "50% advance payment required. Admin will send payment scanner from 7022159756 only." 
  ...
  if (contactBookingForm) {
    ...
    contactBookingForm.addEventListener('submit', (e) => {
      ...
      const branch = document.getElementById('b-branch') ? document.getElementById('b-branch').value : '';
      ...
      const address = document.getElementById('b-address') ? document.getElementById('b-address').value : '';
      const landmark = document.getElementById('b-landmark') ? document.getElementById('b-landmark').value : '';
      const city = document.getElementById('b-city') ? document.getElementById('b-city').value : '';
